...
public interface UzcCrypto {
    ...
    /**
     * Derive the uzc address of the passphrase
     * @param passphrase The passphrase
     * @return The uzc address
     */
    UzcAddress getUzcAddressFromPassphrase(String passphrase);

    /**
     * Derive the uzc address of the private key
     * @param privateKey The private key
     * @return The uzc address
     */
    UzcAddress getUzcAddressFromPrivate(byte[] privateKey);

    /**
     * Derive the uzc address of the public key
     * @param publicKey The public key
     * @return The uzc address
     */
    UzcAddress getUzcAddressFromPublic(byte[] publicKey);

    /**
     * Convert a full hash to an ID
     * @param hash The hash of an account, block, transaction etc
     * @return The Uzc ID of the account, block, transaction etc
     * @throws IllegalArgumentException if the hash is invalid
     */
    UzcID hashToId(byte[] hash) throws IllegalArgumentException;
     ...
     * @return The encrypted message
     */
    UzcEncryptedMessage encryptBytesMessage(byte[] message, String myPassphrase, byte[] theirPublicKey);
     ...
     * @return The encrypted message
     */
    UzcEncryptedMessage encryptBytesMessage(byte[] message, byte[] myPrivateKey, byte[] theirPublicKey);
     ...
     * @return The encrypted message
     */
    UzcEncryptedMessage encryptTextMessage(String message, String myPassphrase, byte[] theirPublicKey);
     ...
     * @return The encrypted message
     */
    UzcEncryptedMessage encryptTextMessage(String message, byte[] myPrivateKey, byte[] theirPublicKey);
     ...
     * @return The plaintext message
     */
    byte[] decryptMessage(UzcEncryptedMessage message, String myPassphrase, byte[] theirPublicKey);
     ...
     * @return The plaintext message
     */
    byte[] decryptMessage(UzcEncryptedMessage message, byte[] myPrivateKey, byte[] theirPublicKey);

    /**
     * Get the Reed-Solomon encoding of a Uzc address. Does not include the "UZC-" prefix.
     * @param uzcID The Uzc ID
     * @return The Reed-Solomon encoding of the address, not including the "UZC-" prefix.
     */
    String rsEncode(UzcID uzcID);

    /**
     * Get the account ID associated with a Reed-Solomon encoded address
     * @param rs The Reed-Solomon encoded address, not including the "UZC-" prefix
     * @return The ID of that RS address
     * @throws IllegalArgumentException If the RS address was invalid and could not be decoded.
     */
    UzcID rsDecode(String rs) throws IllegalArgumentException;

    /**
     * Translate an epoch time to a Java Date object
     * @param epochTime The number of seconds since the Uzc epoch
     ...
     * @return The generation signature of the block
     */
    byte[] calculateGenerationSignature(byte[] lastGenSig, UzcAddress lastGenerator);
     ...
     * @return The hit of that scoop
     */
    BigInteger calculateHit(UzcAddress accountId, long nonce, byte[] genSig, int scoop, int pocVersion);
     ...
     * @return The hit of that scoop
     */
    BigInteger calculateHit(UzcAddress accountId, long nonce, byte[] genSig, byte[] scoopData);
     ...
     * @return The hit of that scoop
     */
    BigInteger calculateDeadline(UzcAddress accountId, long nonce, byte[] genSig, int scoop, long baseTarget, int pocVersion);
     ...
     * @return
     */
    byte[] getATCreationBytes(short atVersion, byte[] code, byte[] data, int dPages, int csPages, int usPages, UzcValue minActivationAmount);

    /**
     * Get a singleton instance of this class
     * @return Singleton instance of this class.
     */
    static UzcCrypto getInstance() {
        return UzcCryptoImpl.INSTANCE;
    }
}
